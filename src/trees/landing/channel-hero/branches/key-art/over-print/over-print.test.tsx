...
import { ChannelHeroOverPrint } from "./over-print";
 ...
 */
describe("ChannelHeroOverPrint", () => {
  it("renders the default fixture with visible content", () => {
    const { container } = render(<ChannelHeroOverPrint {...ALL_FIXTURES[DEFAULT_FIXTURE]} />);
    expect(container.textContent?.trim().length ?? 0).toBeGreaterThan(0);
  });

  it("renders every fixture without throwing", () => {
    for (const [name, vm] of Object.entries(ALL_FIXTURES)) {
      expect(() => render(<ChannelHeroOverPrint {...vm} />), `fixture: ${name}`).not.toThrow();
      cleanup();
    }
  });
