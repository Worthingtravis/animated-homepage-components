...
import { clampProgress, resolveAuroraHeadlineVMState, type AuroraHeadlineVM } from "./aurora-headline.vm";
 ...
 */
export function frameAt(progress: number, overrides: Partial<AuroraHeadlineVM> = {}): AuroraHeadlineVM {
  const clamped = clampProgress(progress);
  return fixture({
    progress: clamped,
    state: resolveAuroraHeadlineVMState(BASE.items.length, clamped),
    ...overrides,
  });
}
